...
```{r}
plot(f_data)
grid.fdata = seq(0,1, length.out = 101)
lines(grid.fdata, median_curve$values, col="black")
library(DepthProc)
tukey.depth=depth(u=f_data$values,method='Tukey')
tukey.deepest.idx = which(tukey.depth==max(tukey.depth))
lines(grid.fdata, f_data$values[tukey.deepest.idx[1],], col="red")

```

It does not seem like a correct estimator for location... This is due to the fact that as $d$ increases, heuristic approaches are utilised for classical depth measures (such as Tukey depth), yielding poor estimation.
