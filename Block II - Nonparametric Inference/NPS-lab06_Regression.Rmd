...
```{r}
T0_x1 <- abs(summary(result)$coefficients[2,3]) # use coefficients[2,1] for the value
T0_x1
...
$$
Where $\hat{y}_i$ is the fitted value of the reduced model in step ($2$) and $\epsilon_i^{(k)}$ a sample without replacement of the estimated residuals.

5. Compare the value of the statistic in ($2$) with the permutational distribution estimated in ($4$) to yield the p-value.
