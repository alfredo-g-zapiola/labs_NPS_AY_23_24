...
$$
\mathbf{Y}_{i,w} \overset{iid}{\sim} \mathbf{Y}_{w} \in \mathbb{R}^{48}; \, i \in \{1,...,N\} \& w \in \{1, ..., 5\}
$$ 
where $w$ denotes the weekday,and $N$ the sample size. We also have:
$$
\mathbf{Y}_{i,\tilde{w}} \overset{iid}{\sim} \mathbf{Y}_{\tilde{w}} \in \mathbb{R}^{48}
$$
where $\tilde{w}$ is weekend day.
...
We're still in Milan, but now we want to check if the humidity of summer months is significantly different from the "non-comfort" threshold level of $(65\%)$.
...
$$
H_0: \mathbf{Y} = \mathbf{\mu}_{0}\;vs\;H_1:\mathbf{Y}_ \neq \mathbf{\mu}_0
$$
Let's read and plot the data also here.
We will have 4 observations for 7 different years.
